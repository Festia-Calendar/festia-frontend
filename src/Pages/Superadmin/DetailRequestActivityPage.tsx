/**
 * คำอธิบาย : Component สำหรับหน้าจอแสดงรายละเอียดคำขออนุมัติกิจกรรม (Detail Request Activity Page)
...
/**
 * คำอธิบาย : ฟังก์ชัน Component หลักสำหรับหน้ารายละเอียดคำขออนุมัติกิจกรรม
 * Input: -
 * Output: UI รายละเอียดคำขออนุมัติกิจกรรม ปุ่มดำเนินการอนุมัติ/ปฏิเสธ และ Modal ยืนยันการทำงาน
 */
export default function DetailRequestActivityPage() {
  ...
  // State สำหรับ Modal ดูรูปใหญ่ของส่วนกำหนดการ
  ...
  /**
   * คำอธิบาย : Hook สำหรับดึงข้อมูลรายละเอียดคำขออนุมัติกิจกรรมจาก Backend ตาม ID
   * Input: -
   * Output: -
   */
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        const data = await activityService.getRequestActivityById(id);
        setActivity(data);
      } catch (error) {
  ...
  // ================= Formatting Helpers (เหมือนหน้า DetailActivityPage) =================
  ...
  /**
   * คำอธิบาย : ฟังก์ชันจัดการการอนุมัติคำขอกิจกรรม ผ่านทาง activityService
   * Input: -
   * Output: -
   */
  const handleApprove = async () => {
    ...
    try {
      await activityService.approveActivity(Number(id));
  ...
  /**
   * คำอธิบาย : ฟังก์ชันจัดการการปฏิเสธคำขอกิจกรรม พร้อมเหตุผล ผ่านทาง activityService
   * Input: -
   * Output: -
   */
  const handleReject = async () => {
    ...
    setIsProcessing(true);
    try {
      await activityService.rejectActivity(Number(id), rejectReason);
  ...
  if (!activity) return <p className="text-center text-gray-500 mt-10">ไม่พบข้อมูลคำร้องขออนุมัติกิจกรรม</p>;

  // กรองสื่อทั้งหมดของกิจกรรม
  ...
  return (
    <div className="w-full max-w-[1100px] mx-auto space-y-8 relative pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        {/* รูปซ้ายมือ (ส่งฟังก์ชัน getImageUrl และ isVideoFile เข้าไปเหมือนหน้า DetailActivityPage) */}
          ...
          <div>
            <div className="flex justify-between items-start mb-3">
              ...
              <div className="flex items-center space-x-2 shrink-0 mt-1">
                <button 
                  onClick={() => { setRejectReason(''); setModalState('rejectReason'); }} 
                  className="border border-gray-300 text-gray-600 px-4 py-2 rounded-lg text-sm font-medium hover:border-[#712874] hover:text-[#712874] transition-colors bg-white shadow-sm cursor-pointer"
                >
                  ปฏิเสธ
                </button>
                <button 
                  onClick={() => setModalState('confirmApprove')} 
                  className="bg-[#712874] text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-purple-900 transition-colors shadow-sm cursor-pointer"
                >
                  อนุมัติ
                </button>
              </div>
      ...
      {/* ใช้ Modal สำหรับอนุมัติ / ปฏิเสธ */}
      <Modal 
